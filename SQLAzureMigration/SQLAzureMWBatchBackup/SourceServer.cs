...
namespace SQLAzureMWBatchBackup
{
    public class SourceServer
    {
        ...
        private SqlSmoObject[] GetAssemblies(Database sourceDatabase)
        {
            List<SqlSmoObject> assemblies = new List<SqlSmoObject>();
            if (sourceDatabase.Parent.Edition == "SQL Azure") return assemblies.ToArray();

            try
            {
                if (_ObjectSelector.Assemblies.Script)
                {
                    if (sourceDatabase.Assemblies.Count > 0)
                    {
                        foreach (SqlAssembly asm in sourceDatabase.Assemblies)
                        {
                            if (asm.IsSystemObject) continue;

                            if (OkToScript(_ObjectSelector.Assemblies, asm.Owner, asm.Name))
                            {
                                assemblies.Add(asm);
                            }
                        }
        ...
        private SqlSmoObject[] GetPartitionFunctions(Database sourceDatabase)
        {
            List<SqlSmoObject> functions = new List<SqlSmoObject>();
            try
            {
                if (_ObjectSelector.PartitionFunctions.Script)
                {
                    try
                    {
                        if (sourceDatabase.PartitionFunctions.Count > 0)
                        {
                            foreach (PartitionFunction fun in sourceDatabase.PartitionFunctions)
                            {
                                if (OkToScript(_ObjectSelector.PartitionFunctions, "", fun.Name))
                                {
                                    functions.Add(fun);
                                }
                            }
        ...
        private SqlSmoObject[] GetPartitionSchemes(Database sourceDatabase)
        {
            List<SqlSmoObject> schemes = new List<SqlSmoObject>();
            try
            {
                if (_ObjectSelector.PartitionSchemes.Script)
                {
                    try
                    {
                        if (sourceDatabase.PartitionSchemes.Count > 0)
                        {
                            foreach (PartitionScheme scheme in sourceDatabase.PartitionSchemes)
                            {
                                if (OkToScript(_ObjectSelector.PartitionSchemes, "", scheme.Name))
                                {
                                    schemes.Add(scheme);
                                }
                            }
        ...
        private SqlSmoObject[] GetSynonyms(Database sourceDatabase)
        {
            List<SqlSmoObject> synonyms = new List<SqlSmoObject>();
            try
            {
                if (_ObjectSelector.Synonyms.Script)
                {
                    if (sourceDatabase.Synonyms.Count > 0)
                    {
                        foreach (Synonym syn in sourceDatabase.Synonyms)
                        {
                            if (OkToScript(_ObjectSelector.Synonyms, syn.Schema, syn.Name))
                            {
                                synonyms.Add(syn);
                            }
                        }
        ...
        private void ScriptDrops(SqlSmoObject[] sorted, SqlSmoObject[] smoRoles, SqlSmoObject[] smoSchemas, SqlSmoObject[] smoSchemasCols, SqlSmoObject[] smoUDTs, SqlSmoObject[] smoUDTTs, SqlSmoObject[] smoAssemblies, SqlSmoObject[] smoSynonyms, SqlSmoObject[] smoPartitionFunctions, SqlSmoObject[] smoPartitionSchemes, ScriptDatabase sdb)
        {
            ...
            if (smoSchemasCols.Count<SqlSmoObject>() > 0) sdb.ScriptDrops(smoSchemasCols);
            ...
            if (smoPartitionSchemes.Count<SqlSmoObject>() > 0) sdb.ScriptDrops(smoPartitionSchemes);
        }
        ...
        public void GenerateScriptFromSourceServer(Server sourceServer, Database sourceDatabase, StreamWriter swTSQL, AsyncUpdateStatus updateStatus, string outputDir, ObjectSelector objectFilter)
        {
                ...
                Regex.IsMatch(_smoScriptOpts.ScriptDropCreate, _smoScriptOpts.GetLocalizedStringValue("SOSDropCreate"), RegexOptions.IgnoreCase))
            {
                ...
                {
                    if (sp.Process(DatabaseObjectsTypes.Assemblies, smoAssemblies, 3)) return;
                    if (sp.Process(DatabaseObjectsTypes.PartitionFunctions, smoPartitionFunctions, 6)) return;
                    if (sp.Process(DatabaseObjectsTypes.PartitionSchemes, smoPartitionSchemes, 9)) return;
                    if (sp.Process(DatabaseObjectsTypes.Roles, smoRoles, 12)) return;
                    if (sp.Process(DatabaseObjectsTypes.Schemas, smoSchemas, 15)) return;
                    if (sp.Process(DatabaseObjectsTypes.XMLSchemaCollections, smoSchemaCols, 20)) return;
                    if (sp.Process(DatabaseObjectsTypes.UserDefinedDataTypes, smoUDTs, 25)) return;
                    if (sp.Process(DatabaseObjectsTypes.UserDefinedTableTypes, smoUDTTs, 30)) return;
                    if (sp.Process(DatabaseObjectsTypes.Synonyms, smoSynonyms, 35)) return;
                }
                if (sp.Process(sorted, 40)) return;
